...
\author{
  Guy Levy\\
  \texttt{replace-with-final-affiliation}\\
  \texttt{replace-with-final-email}
}
...
The SCIP-style index for \texttt{django/forms/models.py} includes occurrences such as references to \texttt{ValidationError}, \texttt{ValidationError\#code}, and \texttt{ValidationError\#message}. This already tells us that \texttt{django/forms/models.py} imports and uses \texttt{ValidationError}, which makes it structurally relevant. But it does not yet tell us whether this is the correct implementation file or merely an adjacent consumer.
...
For publication, these materials should point to a public GitHub repository for code, manifests, and report-generation scripts, such as \url{https://github.com/LevyGuy/graph-context-is-not-enough}, together with an archived artifact snapshot, for example on Zenodo, for the exact benchmark outputs used in the paper.
